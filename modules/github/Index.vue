<template>
  <section-cmp class="configs-root" v-if="service" :key="service.label">
      ...
      <div class="chooser" v-if="!currentPr">
        ...
        <TableGeneric :headers="[
          ...
          <template #assignees="{item: assignees}">
            <div class="avatars">
              <img v-for="user of assignees" class="avatar" :src="user.avatar_url" :alt="user.login" :title="user.login">
            </div>
          </template>
          <template #requested_reviewers="{item: requested_reviewers }">
            <div class="avatars">
              <img v-for="user of requested_reviewers" class="avatar" :src="user.avatar_url" :alt="user.login" :title="user.login">
            </div>
          </template>
        </TableGeneric>
      </div>
    ...
    <div class="container" v-else>
      <h3>Not ready</h3>
      <div>
        <input type="text" v-model="apikey" @keypress.enter="validateApikey" placeholder="Type your github token here...">
        <button @click="validateApikey">Validate</button>
      </div>
    </div>
  </section-cmp>
...
const prToDisplay = computed(() => {
  let res = prs.value
  if(onlyToReview.value) res = res.filter(pr => pr.requested_reviewers.find(user => user.login === loggedAs.value))
